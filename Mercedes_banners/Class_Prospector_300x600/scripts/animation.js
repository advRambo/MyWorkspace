...
function slide_in_left(a) {
	return new TimelineMax()
		.to(a, 12, { x: '-=271px', ease: Power0.easeOut, force3D: false });
}


function zoom_in(a) {
	zoom_tl = new TimelineMax()
		.to(a, 10, { scale: 1.08, x: '-=0px', transformOrigin: '45% 50%', rotation: 0.01, ease: Power1.easeOut, force3D: false });
}

function zoom_in_delay(a) {
	zoom_tl = new TimelineMax()
		.to(a, 10, { delay: 1, scale: 1.08, x: '-=0px', transformOrigin: '20% 50%', rotation: 0.01, ease: Power1.easeOut, force3D: false });
}
...
banner.maintimeline = function () {
	...
	tl
		// .call(zoom_in, [img_01])
		.call(slide_in_left(img_01))
		.add(fadeIn_to(frame1, 0.75))
		.add(slide_in(black_strip, 0.75, '63'))
		.add(fadeIn([cta_container, hover_link], 0.75), '-=0.5')

		.add(fadeIn_up(copy_01), '+=1')

		// .call(slide_in, (img_01, 0.75, '-300'))
		.add(fadeOut(copy_01, 0.75), '+=2')


		// .add(fadeIn(img_02,1),'-=0.75')
		...
		.add(fadeOut([frame1, cta_container, black_strip], 0.75), '+=5')
		;
}
